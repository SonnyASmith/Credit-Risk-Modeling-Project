"""
Unified Credit Risk Model Framework with Threshold Optimization

This module provides a unified framework for training, evaluating, and comparing
different credit risk models using a consistent approach, including threshold optimization.
...
FEATURES = ["dti", "credit_age_years", "payment_to_income_ratio",
            "revol_util", "fico_squared", "payment_ratio_to_loan", "loan_to_income_ratio",
            "inq_last_6mths", "delinq_2yrs", "collections_12_mths_ex_med","fico_score", "fico_dti_interaction"]
...
class CreditDataProcessor:
    ...
    def __init__(self, data_path="feature_data.csv", target_col="loan_paid"):
        """
...
class CreditModelTrainer:
    ...
    # MODIFIED METHOD: Added threshold parameter
    def predict(self, X, threshold=None):
        ...
        Args:
            ...
            threshold: Custom probability threshold for positive class
                      If None, uses default 0.5 threshold

        Returns:
            y_pred: Class predictions (0 = paid off, 1 = not paid off)
        """
        # If no custom threshold provided, use standard predict method
        if threshold is None or not hasattr(self.model, 'predict_proba'):
            return self.model.predict(X)

        # With custom threshold, get probabilities and apply threshold
        try:
            y_pred_proba = self.predict_proba(X)
            return (y_pred_proba >= threshold).astype(int)
        except (AttributeError, NotImplementedError):
            print(f"Warning: Model {self.model_name} doesn't support probability predictions. Using default predict.")
            return self.model.predict(X)
...
class CreditModelEvaluator:
    ...
    for credit risk models with threshold optimization capabilities.
    """
    ...
    def evaluate_model(self, model_trainer, X_test, y_test, model_name=None, threshold=None):
        """
        ...
        Args:
            ...
            model_name (str): Name to identify this model
            threshold (float): Custom threshold for predictions (optional)
        ...
        # Get predictions
        y_pred = model_trainer.predict(X_test, threshold=threshold)
        ...
        # Store the model name with threshold if provided
        result_key = model_name
        if threshold is not None:
            result_key = f"{model_name}_t{threshold}"

        # Store results for later comparison
        self.evaluation_results[result_key] = metrics

        return metrics

    # NEW METHOD: Evaluate model at different thresholds
    def evaluate_thresholds(self, model_trainer, X_test, y_test, model_name=None,
                            thresholds=None, eval_metrics=None):
        """
        ...
        print(f"\n===== THRESHOLD EVALUATION: {model_name} =====")

        # Check if model supports probability predictions
        try:
            y_pred_proba = model_trainer.predict_proba(X_test)
        except (AttributeError, NotImplementedError):
            print(f"Model {model_name} doesn't support probability predictions. Cannot evaluate thresholds.")
            return None
        ...
        # Store in evaluation results with special key
        self.evaluation_results[f"{model_name}_thresholds"] = results_df
    ...
    # NEW METHOD: Find optimal threshold based on criteria
    def find_optimal_threshold(self, model_name, criterion='f1', target_value=None):
        """
        Find the optimal threshold based on specified criterion.

        Args:
            model_name (str): Name of the model
            criterion (str): Criterion to optimize ('f1', 'precision', 'recall', 'specificity', 'balanced')
                            If 'balanced', maximizes the sum of precision and recall
                            If 'custom_cost', minimizes FP*fp_cost + FN*fn_cost
            target_value (float): Target value for precision or recall
                                 If provided, finds threshold that achieves the target value

        Returns:
            float: Optimal threshold value
        """
        threshold_key = f"{model_name}_thresholds"
        if threshold_key not in self.evaluation_results:
            raise ValueError(f"No threshold evaluation results for {model_name}. "
                             f"Run evaluate_thresholds() first.")

        results_df = self.evaluation_results[threshold_key]

        # If target value is provided for precision or recall
        if target_value is not None and criterion in ['precision', 'recall']:
            # Find threshold that exceeds target value
            filtered_df = results_df[results_df[criterion] >= target_value]
            if filtered_df.empty:
                print(f"No threshold achieves target {criterion} of {target_value}")
                return None

            # For precision, return the lowest threshold that meets target (to maximize recall)
            if criterion == 'precision':
                return filtered_df['threshold'].min()
            # For recall, return the highest threshold that meets target (to maximize precision)
            else:
                return filtered_df['threshold'].max()

        # Otherwise find threshold that maximizes the criterion
        if criterion == 'balanced':
            # Sum of precision and recall
            results_df['balanced'] = results_df['precision'] + results_df['recall']
            criterion = 'balanced'
        elif criterion == 'custom_cost':
            # Use custom cost function (default: FN costs 5x more than FP)
            fp_cost = 1  # Cost of false positive
            fn_cost = 5  # Cost of false negative
            results_df['cost'] = (results_df['fp'] * fp_cost + results_df['fn'] * fn_cost) / len(results_df)
            # Find threshold that minimizes cost
            optimal_idx = results_df['cost'].idxmin()
            return results_df.loc[optimal_idx, 'threshold']

        # Find threshold that maximizes the criterion
        optimal_idx = results_df[criterion].idxmax()
        return results_df.loc[optimal_idx, 'threshold']

    # NEW METHOD: Plot threshold performance curves
    def plot_threshold_metrics(self, model_name, metrics=None, ax=None, save=True):
        """
        ...
        Args:
            ...
            metrics (list): List of metrics to plot
                          If None, uses ['precision', 'recall', 'f1', 'accuracy']
        ...
        for metric in metrics:
            if metric in results_df.columns:
                ax.plot(results_df['threshold'], results_df[metric], marker='o', label=metric)
        ...
        # Save if requested
        if save:
            filepath = f"{self.output_dir}/{model_name.lower().replace(' ', '_')}_threshold_metrics.png"
            plt.savefig(filepath)
            print(f"Threshold metrics plot saved to {filepath}")
    ...
    def plot_confusion_matrix(self, model_name=None, ax=None, cmap='Blues', save=True):
        ...
        # Add threshold to title if available
        if 'threshold' in self.evaluation_results[model_name]:
    ...
    def plot_roc_curve(self, model_names=None, ax=None, save=True):
        ...
        for model_name in model_names:
            ...
            # Add threshold markers if this is a single model plot
            if len(model_names) == 1:
                # Mark standard thresholds (0.3, 0.5, 0.7)
                threshold_markers = [0.3, 0.5, 0.7]
                for t in threshold_markers:
                    idx = np.abs(thresholds - t).argmin()
                    ax.plot(fpr[idx], tpr[idx], 'ro', markersize=5)
                    ax.annotate(f"{t}", (fpr[idx], tpr[idx]),
    ...
    def plot_precision_recall_curve(self, model_names=None, ax=None, save=True):
        ...
        for model_name in model_names:
            ...
            # Add threshold markers if this is a single model plot
            if len(model_names) == 1 and len(thresholds) > 0:
                # Mark standard thresholds (0.3, 0.5, 0.7)
                threshold_markers = [0.3, 0.5, 0.7]
                for t in threshold_markers:
                    if t <= max(thresholds):
                        ...
                        if idx < len(precision) - 1:  # Ensure index is valid
                            ax.plot(recall[idx + 1], precision[idx + 1], 'ro', markersize=5)
                            ax.annotate(f"{t}", (recall[idx + 1], precision[idx + 1]),
                                        textcoords="offset points", xytext=(10, 5))
    ...
    def compare_models(self, metric='f1'):
        ...
        # Include threshold in comparison if available
        metrics.append('threshold')

        # Create comparison DataFrame
        comparison = {}
        for model_name, results in self.evaluation_results.items():
            # Skip threshold evaluation results
            if "_thresholds" in model_name:
                continue
    ...
    def plot_model_comparison(self, metrics=None, ax=None, save=True):
        ...
        # Filter to requested metrics and ensure they exist
    ...
    def plot_confusion_matrices_comparison(self, model_names=None, save=True):
        ...
        if model_names is None:
            # Skip threshold evaluation results
            model_names = [name for name in self.evaluation_results.keys()
                           if "_thresholds" not in name]
        elif isinstance(model_names, str):
            model_names = [model_names]
...
if __name__ == "__main__":
    ...
    # NEW: Evaluate Random Forest model at different thresholds
    evaluator.evaluate_thresholds(rf_trainer, X_test, y_test, model_name='RandomForest')

    # NEW: Plot threshold metrics
    evaluator.plot_threshold_metrics(model_name='RandomForest')

    # NEW: Find and apply optimal threshold for precision (e.g., target 0.8)
    optimal_precision_threshold = evaluator.find_optimal_threshold('RandomForest',
                                                                   criterion='precision',
                                                                   target_value=0.8)
    print(f"\nOptimal threshold for 80% precision: {optimal_precision_threshold:.2f}")
    evaluator.evaluate_model(rf_trainer, X_test, y_test,
                             model_name='RandomForest_Precision80',
                             threshold=optimal_precision_threshold)

    # NEW: Find and apply optimal F1 threshold
    optimal_f1_threshold = evaluator.find_optimal_threshold('RandomForest', criterion='f1')
    print(f"\nOptimal threshold for F1 score: {optimal_f1_threshold:.2f}")
    evaluator.evaluate_model(rf_trainer, X_test, y_test,
                             model_name='RandomForest_OptimalF1',
                             threshold=optimal_f1_threshold)

    # Regular evaluation plots
    evaluator.plot_confusion_matrix(model_name='RandomForest')
    evaluator.plot_roc_curve(model_names='RandomForest')
    evaluator.plot_precision_recall_curve(model_names='RandomForest')
    ...
    # NEW: Evaluate Logistic Regression model at different thresholds
    evaluator.evaluate_thresholds(lr_trainer, X_test, y_test, model_name='LogisticRegression')

    # NEW: Plot threshold metrics
    evaluator.plot_threshold_metrics(model_name='LogisticRegression')

    # NEW: Find and apply optimal recall threshold (e.g., target 0.85)
    optimal_recall_threshold = evaluator.find_optimal_threshold('LogisticRegression',
                                                                criterion='recall',
                                                                target_value=0.85)
    print(f"\nOptimal threshold for 85% recall: {optimal_recall_threshold:.2f}")
    evaluator.evaluate_model(lr_trainer, X_test, y_test,
                             model_name='LogisticRegression_Recall85',
                             threshold=optimal_recall_threshold)

    # Regular evaluation plots
    evaluator.plot_confusion_matrix(model_name='LogisticRegression')
    evaluator.plot_roc_curve(model_names='LogisticRegression')
    ...
    # NEW: Evaluate XGBoost model at different thresholds
    evaluator.evaluate_thresholds(xgb_trainer, X_test, y_test, model_name='XGBoost')

    # NEW: Plot threshold metrics
    evaluator.plot_threshold_metrics(model_name='XGBoost')

    # NEW: Find and apply balanced threshold (maximize precision + recall)
    optimal_balanced_threshold = evaluator.find_optimal_threshold('XGBoost', criterion='balanced')
    print(f"\nOptimal balanced threshold: {optimal_balanced_threshold:.2f}")
    evaluator.evaluate_model(xgb_trainer, X_test, y_test,
                             model_name='XGBoost_Balanced',
                             threshold=optimal_balanced_threshold)

    # NEW: Find and apply custom cost-based threshold
    optimal_cost_threshold = evaluator.find_optimal_threshold('XGBoost', criterion='custom_cost')
    print(f"\nOptimal cost-based threshold: {optimal_cost_threshold:.2f}")
    evaluator.evaluate_model(xgb_trainer, X_test, y_test,
                             model_name='XGBoost_CostOptimized',
                             threshold=optimal_cost_threshold)

    # Regular evaluation plots
    evaluator.plot_confusion_matrix(model_name='XGBoost')
    evaluator.plot_roc_curve(model_names='XGBoost')
    ...
    # NEW: Compare optimal threshold models
    optimal_models = ['RandomForest_OptimalF1', 'LogisticRegression_Recall85', 'XGBoost_CostOptimized']
    evaluator.plot_roc_curve(model_names=optimal_models)
    evaluator.plot_precision_recall_curve(model_names=optimal_models)
    evaluator.plot_confusion_matrices_comparison(model_names=optimal_models)

    # Regular model comparisons
